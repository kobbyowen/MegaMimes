...
static int assert_magic_confidence(const unsigned char* bytes, size_t n) {
  MegaMimesCtx* ctx = mega_open(NULL);
  if (!ctx) return 1;
  char* path = write_temp_bytes("mm-magic-conf", bytes, n);
  if (!path) { mega_close(ctx); return 1; }
  ...
  int fail = (rc != MEGA_OK)
    ...
    || info->confidence < 0.90f
    || strcmp(info->source, "magic") != 0
...
int test_magic_suite(void) {
  ...
  fails += assert_magic_confidence(pdf, sizeof(pdf));
  return fails;
}