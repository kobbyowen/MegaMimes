...
/* Table format: { ext, name, mime, reserved } — reserved is "0" for now */
static const struct { const char* ext; const char* name; const char* mime; const char* reserved; } TBL[] = {
...
/* return pointer to the extension within 'path' (without the dot), or NULL */
static const char* find_extension(const char* path) {
  if (!path) return NULL;
  /* get basename (ignore dots in directories) */
  const char* base = path;
  for (const char* p = path; *p; ++p) {
    if (*p == '/' || *p == '\\') base = p + 1;
  }
  /* last dot in basename */
  const char* last_dot = NULL;
  for (const char* p = base; *p; ++p) {
    if (*p == '.') last_dot = p;
  }
  if (!last_dot || *(last_dot + 1) == '\0') return NULL; /* no ext or trailing dot */
  return last_dot + 1;
}

/* exported to megamimes.c */
const char* mega_ext_lookup(const char* path, const char** out_name) {
  const char* e = find_extension(path);
  if (!e) return NULL;

  for (int i = 0; TBL[i].ext; ++i) {
    if (MEGA_STRCASECMP(e, TBL[i].ext) == 0) {
      if (out_name) *out_name = TBL[i].name;  /* name from the 2nd field */
      return TBL[i].mime;                     /* mime from the 3rd field */
    }
  }
  return NULL;
}
